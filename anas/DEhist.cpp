...
int main(int argc, char *argv[])
{
  ...
  bottom->MakeHists("histogram_files/de_bottom.root",bottom->BottomArea(),0,0);
  delete bottom;

  numi::DistEnergy *length = new numi::DistEnergy(dir_length,genie_file,file_list,false,true,false);
  length->MakeHists("histogram_files/de_length.root",length->LengthArea(),0,0);
  delete length;

  numi::DistEnergy *normal = new numi::DistEnergy(dir_normal,genie_file,file_list,true,false,false);
  normal->MakeHists("histogram_files/de_normal.root",normal->NormalArea(),0,0);
  delete normal;
